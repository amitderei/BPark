...
public class Server extends AbstractServer {
	...
	/** Reference to the ServerController, used for log updates */
	private ServerController serverController;

	/**
	 * Sets the ServerController instance for this Server.
	 *
	 * @param controller the ServerController to link with this Server
	 */
	...
	protected void handleMessageFromClient(Object msg, ConnectionToClient client) {
		try {
			...
			case DISCONNECT:
				...
				serverController.addLog("Client disconnected: [" + host + "] IP: " + ip + " ID: " + id);
			...
			case DELETE_ORDER:
				...
				if (succeed) {
					serverController.addLog("Order number: "+ orderNumberToDelete +" deleted successfully.");
			...
			case FORGEOT_MY_PARKING_CODE:
				try {
					...
					serverController.addLog("email sent successfully to: "+ email);
			...
			case UPDATE_DETAILS_OF_SUBSCRIBER:
				...
				if (isSucceedSubscriber && isSucceedUser) {
					serverController.addLog("Updated details of subscriber: " + (Subscriber) data[1]);
			...
			case ADD_NEW_ORDER:
				...
				if (success) {
					serverController.addLog("Added order number: " + orderToAdd.getOrderNumber());
			...
			case DELIVER_VEHICLE:
				...
				serverController.addLog("Subscriber " + newParkingEvent.getSubscriberCode() + " has delivered his vehicle");
